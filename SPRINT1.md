# Sprint 1

## Objetivo

Desarrollar la estructura inicial de la aplicación de control de despensa y presupuesto del hogar.

## Historias de usuario

- HU-01 Registro e inicio de sesión.
- HU-02 Presupuesto mensual.
- HU-03 Registro de productos.
- HU-05 Buscador y filtro (parcial).

## Actividades realizadas

- Creación del repositorio.
- Definición de estructura del proyecto.
- Diseño inicial de pantallas.
- Planeación de componentes Vue.js.
- Elaboración del Product Backlog.

## Entregables

- Planeación del Sprint.
- Estructura inicial del proyecto.
- Diseño preliminar de interfaces.
  
## Resultados obtenidos

- Estructura inicial del proyecto definida.
- Historias de usuario identificadas.
- Product Backlog generado.
- Planeación del Sprint completada.
